...
namespace ImageScan
{
    class XmlReader
    {
        ...
        #region ReadXmlString_AttachDetachMulti

        /// <summary>
        /// 
        /// </summary>
        /// <param name="strXml"></param>
        /// <param name="arScanr"></param>
        /// <param name="sStatus"></param>
        public void ReadXmlString_AttachDetachMulti(string strXml, out Scanner[] arScanr, out string sStatus)
        {
            arScanr = new Scanner[5];

            for (int i = 0; i < 5; i++)
            {
                arScanr.SetValue(null, i);
            }
        ...
        #region GetAttributePos

        /// <summary>
        /// 
        /// </summary>
        /// <param name="strNumber"></param>
        /// <param name="scanr"></param>
        ...
        private void GetAttributePos(string strNumber, Scanner scanr, out int nIndex)
        {
            ...
            {
                string strInNumber = scanr.m_arAttributes.GetValue(i, Scanner.POS_ATTR_ID).ToString();
                                ...
                                else if (null != scanr)
                                {
                                    switch (sElementName)
                                    {
                                        case Scanner.TAG_OPCODE:
                                            ...
                                            if (!(ScannerDefinitions.RSM_ATTR_GETALL == nOpCode ||
                                                    ScannerDefinitions.RSM_ATTR_GET == nOpCode ||
                                                    ScannerDefinitions.RSM_ATTR_GETNEXT == nOpCode))
                                            {
                                                return;
                                            }
                                            break;
        ...
        #region Clear_scanner_attributes
        ...
        public void Clear_scanner_attributes(Scanner scanr)
        {
            try
            {
                ...
                {
                    scanr.m_arAttributes.SetValue(null, i, Scanner.POS_ATTR_NAME);
                    scanr.m_arAttributes.SetValue(null, i, Scanner.POS_ATTR_ID);
                    scanr.m_arAttributes.SetValue(null, i, Scanner.POS_ATTR_TYPE);
                    scanr.m_arAttributes.SetValue(null, i, Scanner.POS_ATTR_PROPERTY);
                    scanr.m_arAttributes.SetValue(null, i, Scanner.POS_ATTR_VALUE);
                }
            }
            catch (Exception)
            {
